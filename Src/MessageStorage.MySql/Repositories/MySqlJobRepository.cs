...
WHERE jo.job_status = @source_job_status
  and jo.execute_later_than < @source_execute_later_than
ORDER BY jo.id
LIMIT 0,1
FOR UPDATE);
...
select j.id
     , j.message_handler_type_name
     , j.created_on as job_created_on
     , j.job_status
     , j.last_operation_time
     , j.last_operation_info
     , j.current_execution_attempt_count
     , j.execute_later_than
     , j.message_id
     , m.payload
     , m.created_on as message_created_on
from {_creator.RepositoryConfiguration.Schema}.jobs j
         inner join {_creator.RepositoryConfiguration.Schema}.messages m on j.message_id = m.id
where j.id = @jobId
";
