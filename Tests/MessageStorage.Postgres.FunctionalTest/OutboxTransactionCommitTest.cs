...
using MessageStorage.Postgres.FunctionalTest.Fixtures;
...
namespace MessageStorage.Postgres.FunctionalTest
{
...
");
        ...
        public async Task WhenTransactionCommitted__JobIsPersistedInDb()
        {
            ...
            await using var connection = new NpgsqlConnection(_repositoryConfiguration.ConnectionString);
        ...
        public async Task WhenNpgsqlTransactionNotCommitted__JobIsNotPersistedInDb()
        {
            ...
            IEnumerable<Job> jobs;
            ShipmentEntity shipmentEntity;
            await using (NpgsqlTransaction transaction = (NpgsqlTransaction) (await _shipmentDbContext.Database.BeginTransactionAsync()).GetDbTransaction())
            {
                shipmentEntity = new ShipmentEntity("buyer-name");
                _shipmentDbContext.ShipmentEntities.Add(shipmentEntity);
                await _shipmentDbContext.SaveChangesAsync();
        ...
        public class ShipmentDbContext : DbContext
        {
            public ShipmentDbContext(string connectionStr) :
                base(new DbContextOptionsBuilder().UseNpgsql(connectionStr).Options)
            {
            }
