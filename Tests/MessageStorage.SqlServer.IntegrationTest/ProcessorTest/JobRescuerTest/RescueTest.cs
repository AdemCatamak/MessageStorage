...
namespace MessageStorage.SqlServer.IntegrationTest.ProcessorTest.JobRescuerTest;
...
public class RescueTest : IDisposable
{
    ...
    public async Task WhenJobIsInProgressForALongTerm_JobStatusShouldBeSetAsQueued()
    {
        ...
        Job? jobFromDbBefore = await Db.Fetch.JobFromSqlServerAsync(job.Id);
        ...
        Job? jobFromDbAfter = await Db.Fetch.JobFromSqlServerAsync(job.Id);
